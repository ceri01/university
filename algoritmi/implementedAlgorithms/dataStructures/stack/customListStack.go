...
func (stack *CustomListStack) Push(val int) { // Costo tempo => Costante O(1)
	...
	/*
		in questo caso viene utilizzata una funzione con costo di tempo lineare, ma siccome viene cercato sempre
		l'elemento in prima posizione il tempo di esecuzione è sempre lo stesso, quindi costante.
	*/
}
...
func (stack *CustomListStack) Pop() int { // Costo tempo => Costante O(1)
	...
	_, stack.top = stack.list.SearchByPosition(0)
	/*
		Anche in questo caso vengono utilizzate delle funzioni con costo di tempo lineare, ma anche qui viene vincolato
		il caso migliore che ha tempo costante, quindi possiamo dire che il costo di tempo della funzione Dequeue è costante
	*/
	return el
}
